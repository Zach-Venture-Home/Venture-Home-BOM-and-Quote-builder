...
assert.match(html, /equipment-library\.js\?v=2\.4\.0/,'equipment cutout library must load before the app');
...
assert.match(app, /const APP_VERSION='v2.4.0'/,'app release version must be current');
assert.match(html, /styles\.css\?v=2\.4\.0["']/,'stylesheet URL must be cache-busted for the current release');
assert.match(html, /pricing-core\.js\?v=2\.4\.0["']/,'pricing engine URL must be cache-busted for the current release');
assert.match(html, /app\.js\?v=2\.4\.0["']/,'app script URL must be cache-busted for the current release');
