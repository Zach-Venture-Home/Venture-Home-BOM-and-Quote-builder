(function(root,factory){
  ...
  function customerDetailedRows(rows,markup=0.125,laborMultiplier=1.3){
    ...
    const adjustment=effective.filter(row=>row.automatic).reduce((sum,row)=>sum+roundMoney(Number(row.cost)*quantity(row.qty)),0);
    let visible=effective.filter(row=>!row.automatic).map(row=>({...row}));
    const measuredTrenchBase=visible.filter(isMeasuredTrench).reduce((sum,row)=>sum+roundMoney(Number(row.cost)*quantity(row.qty)),0);
    if(adjustment>0&&!measuredTrenchBase) visible.push({item:'Trenching',category:'Trench',cost:adjustment,qty:1});
    const priced=visible.map(row=>{
      let adjustedBase=roundMoney(Number(row.cost)*quantity(row.qty));
      if(isLabor(row)) adjustedBase=roundMoney(adjustedBase*laborMultiplier);
      else if(adjustment>0&&isMeasuredTrench(row)&&measuredTrenchBase>0) adjustedBase=roundMoney(adjustedBase+(adjustment*(adjustedBase/measuredTrenchBase)));
    ...
    return priced.map(({adjustedBase,cents,fraction,...row})=>({...row,customerPrice:roundMoney(cents/100)}));
  }
  ...
  return {TRENCH_MINIMUM,roundMoney,effectiveRows,calculateTotals,customerDetailedRows,validateProject,isTrenchMinimum,isMeasuredTrench};
});
